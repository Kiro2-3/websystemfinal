...
if(isset($_POST['pwdrst'])) {
    ...
    if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Insert a new record into the password_resets table
    ...
    $mail->addAddress($email); // Add a recipient
        ...
        $mail->Body = "Click the following link to reset your password: 
                        <a href='http://localhost/login/password_reset.php?token=$token'>Reset Password</a>";

        try {
            $mail->send();
            $msg = 'Password reset link sent to your email. Check your inbox.';
        } catch (Exception $e) {
            $msg = 'Error sending password reset link. Please try again later.';
        }
...
<!-- 
    STEP 1: Send the email to the user using PHP mailer with their password token attached (forgot.php)
    STEP 2: Line 24 will get the ID, the Email, and the token and check if the token that is generate for them (forgot.php)
    matches with the token inside the email 
    STEP 3: It will check whether the token is still good to use or is it expired already (line 12 password.php)
    STEP 4: After the user provided the new password, it will delete the existing token that has been used to check in the previous steps (line 31 reset_password_process.php)
    STEP 5: Proceed with the login page 
 -->